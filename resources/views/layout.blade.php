      ...
      <div>
        ...
        <ul class="space-y-4">
          @foreach(['ofertas' => 'Oportunidades', 'miembros' => 'Miembros', 'blog' => 'Blog Técnico'] as $route => $label)
            <li>
              <a href="{{ route($route) }}" class="flex items-center group text-sm hover:text-white transition-colors">
                <!-- Indicador (se mantiene el círculo para efecto visual) -->
        ...
        <div class="flex space-x-4">
          ...
          @foreach(['facebook', 'twitter', 'instagram'] as $social)
            <a href="#" class="p-3 bg-tech-dark rounded-lg hover:bg-primary transition-colors">
              <i class="{{ $socialIcons[$social] }} text-gray-300 hover:text-white transition-colors text-xl"></i>
            </a>
          @endforeach
        </div>
      </div>
